...
class ForecastingEngine:
    ...
    def predict_and_recommend(self, rank, category, gender, branch, counselling_round='Phase 1',
        ...
        for code, data in colleges_dict.items():
            ...
            if weight_total > 0:
                historical_cutoff = weighted_sum / weight_total
            else:
                all_ranks = [v['closing'] for v in year_cutoffs.values()]
                historical_cutoff = float(np.mean(all_ranks)) if all_ranks else 2000.0

            # --- Hybrid ML component: use best trained model when available ---
            ml_cutoff = None
            model_used = None
            try:
                if self._trained_models:
                    best = self._trained_models.get(self.best_model_name) or next(iter(self._trained_models.values()))
                    # Feature vector matching training schema
                    is_final = 1 if counselling_round == 'Final Phase' else 0
                    is_girls = 1 if str(gender).upper() in ('GIRLS', 'GIRL', 'FEMALE') else 0
                    # Use group means learned during training (approximate via historical)
                    cat_enc = historical_cutoff  # fallback encoding
                    college_enc = historical_cutoff
                    branch_enc = historical_cutoff
                    # Prefer latest year as target year for forecast
                    target_year = 2026
                    X_pred = pd.DataFrame([{
                        'year': target_year,
                        ...
                        'is_girls': is_girls,
                        'cat_encoded': cat_enc,
                        'college_encoded': college_enc,
                        'branch_encoded': branch_enc
                    }])
                    ml_cutoff = float(best.predict(X_pred)[0])
                    model_used = self.best_model_name
            except Exception:
                ml_cutoff = None

            # Blend: 55% historical weighted + 45% ML when ML available
            if ml_cutoff is not None and ml_cutoff > 0:
                expected_cutoff = int(0.55 * historical_cutoff + 0.45 * ml_cutoff)
            else:
                expected_cutoff = int(historical_cutoff)
            ...
            reasons = []
            if model_used:
                reasons.append(f"Hybrid forecast blends recency-weighted history with {model_used} (expected cutoff ≈ {expected_cutoff:,}).")
        ...
        # -----------------------------------------------------------------
        # ALWAYS show all 4 tiers on every prediction:
        #   Dream <= 20   (hardest — lowest expected cutoff)
        #   Ambitious <= 40
        #   Moderate <= 50
        #   Safe <= 120   (easiest — highest expected cutoff)
        # Real admission_probability is kept; tier labels are assigned by
        # relative competitiveness so results are never "Safe only".
        # -----------------------------------------------------------------
        recommendations.sort(key=lambda x: (x['expected_cutoff'], -x['admission_probability']))
        n = len(recommendations)
        color_map = {
            'Safe': 'green', 'Moderate': 'yellow',
            'Ambitious': 'orange', 'Dream': 'red'
        }

        want = {'Dream': 20, 'Ambitious': 40, 'Moderate': 50, 'Safe': 120}
        total_want = sum(want.values())
        if n < total_want and n >= 4:
            scale = n / float(total_want)
            d = max(1, int(round(want['Dream'] * scale)))
            a = max(1, int(round(want['Ambitious'] * scale)))
            m = max(1, int(round(want['Moderate'] * scale)))
            s = max(1, n - d - a - m)
            want = {'Dream': d, 'Ambitious': a, 'Moderate': m, 'Safe': s}
        elif n < 4:
            labels = ['Dream', 'Ambitious', 'Moderate', 'Safe']
            for i, r in enumerate(recommendations):
                r['classification'] = labels[min(i, 3)]
                r['class_color'] = color_map[r['classification']]
            return recommendations

        idx = 0
        for tier in ('Dream', 'Ambitious', 'Moderate', 'Safe'):
            for _ in range(want[tier]):
                if idx >= n:
                    break
                recommendations[idx]['classification'] = tier
                recommendations[idx]['class_color'] = color_map[tier]
                idx += 1
        while idx < n:
            recommendations[idx]['classification'] = 'Safe'
            recommendations[idx]['class_color'] = color_map['Safe']
            idx += 1

        by_tier = {'Safe': [], 'Moderate': [], 'Ambitious': [], 'Dream': []}
        for r in recommendations:
            by_tier[r['classification']].append(r)
        for tier in by_tier:
            by_tier[tier].sort(key=lambda x: (-x['admission_probability'], x['expected_cutoff']))

        balanced = []
        for tier, quota in [('Safe', 120), ('Moderate', 50), ('Ambitious', 40), ('Dream', 20)]:
            balanced.extend(by_tier[tier][:quota])

        balanced.sort(key=lambda x: (-x['admission_probability'], x['expected_cutoff']))
        return balanced
