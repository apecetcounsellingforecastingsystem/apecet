...
from otp_service import create_and_send_otp, verify_otp, is_target_verified
import config

app = Flask(__name__)
app.secret_key = config.SECRET_KEY
# Harden session cookies for production-style deploys
app.config.update(
    SESSION_COOKIE_HTTPONLY=True,
    SESSION_COOKIE_SAMESITE='Lax',
    # Set SESSION_COOKIE_SECURE=True behind HTTPS (Render/PythonAnywhere HTTPS)
...
def set_security_headers(response):
    """Basic production security headers (drawback #5 mitigation)."""
    response.headers['X-Content-Type-Options'] = 'nosniff'
    response.headers['X-Frame-Options'] = 'SAMEORIGIN'
    response.headers['Referrer-Policy'] = 'strict-origin-when-cross-origin'
    response.headers['X-XSS-Protection'] = '1; mode=block'
    # CSP kept relatively open for CDN Leaflet/Chart.js
...
@app.template_filter('website_url')
def website_url_filter(value):
    """Normalize college website to a clickable https URL. Skips empty or placeholder values."""
    if not value:
        return ''
    v = str(value).strip()
    if not v or v in ('-', 'N/A', 'NA', 'null', 'None', '#'):
        return ''
...
def is_rate_limited(identifier, max_attempts=None, window_minutes=None):
    max_attempts = max_attempts or config.MAX_LOGIN_ATTEMPTS
    window_minutes = window_minutes or config.LOGIN_LOCKOUT_MINUTES
    ...
    cursor.execute(
        ...
        (identifier, f'-{window_minutes} minutes')
    )
...
def log_login_attempt(identifier, success):
    ...
    cursor.execute(
        ...
        (identifier, _client_ip(), 1 if success else 0)
    )
    conn.commit()
    conn.close()
...
# ---------------------------------------------------------------------------
# OTP API endpoints (Feature 1 — Registration verification)
# ---------------------------------------------------------------------------
...
def api_send_otp():
    ...
    if channel not in ('email', 'phone'):
        return jsonify({'success': False, 'message': 'channel must be email or phone'}), 400

    # Simple rate limit on OTP sends
    if is_rate_limited(f'otp:{target}', max_attempts=8, window_minutes=30):
        return jsonify({'success': False, 'message': 'Too many OTP requests. Please wait and try again.'}), 429

    result = create_and_send_otp(target, channel, purpose)
    log_login_attempt(f'otp:{target}', result.get('success', False))
...
def api_verify_otp():
    ...
    result = verify_otp(target, channel, otp_code, purpose)
    if result.get('success'):
        # Persist verification flags in session for registration gate
        if channel == 'email':
            session['email_verified'] = target.lower()
        else:
            session['phone_verified'] = target
    status = 200 if result.get('success') else 400
    return jsonify(result), status
...
def login():
    if request.method == 'POST':
        ...
        if is_rate_limited(email):
            flash(f'Too many failed attempts. Account temporarily locked for {config.LOGIN_LOCKOUT_MINUTES} minutes.', 'danger')
...
def register():
    if request.method == 'POST':
        ...
        # OTP verification gate (email + phone must be verified)
        email_ok = (
            session.get('email_verified') == email
            or is_target_verified(email, 'email', 'register')
        )
        phone_ok = (
            session.get('phone_verified') == phone
            or is_target_verified(phone, 'phone', 'register')
        )
        if not email_ok:
            flash('Please verify your email with OTP before registering.', 'danger')
            return render_template('auth/register.html')
        if not phone_ok:
            flash('Please verify your mobile number with OTP before registering.', 'danger')
            return render_template('auth/register.html')
...
def forgot_password():
    if request.method == 'POST':
        email = request.form.get('email', '').strip().lower()
        if is_rate_limited(f'reset:{email}', max_attempts=5, window_minutes=30):
        ...
        if user:
            ...
            cursor.execute(
                ...
                (generate_password_hash(temp_pw), user['id'])
            )
            conn.commit()
            conn.close()
            log_login_attempt(f'reset:{email}', success=True)

            # Prefer real email when SMTP is configured
            emailed = False
            if config.email_configured():
                try:
                    ...
                    body = (
                        ...
                        f"Your temporary password for AP ECET Counselling System is:\n\n"
                        f"    {temp_pw}\n\n"
                        f"Please login and change it immediately under Change Password.\n\n"
                        f"— AP ECET System"
                    )
                    msg = MIMEText(body)
                    msg['Subject'] = 'AP ECET — Password Reset'
            ...
            if emailed:
                flash('A temporary password has been sent to your email. Login and change it immediately.', 'success')
            else:
                # Demo / offline fallback — show on screen
                flash(
                    f'Recovery successful. Temporary password: {temp_pw} — login and change it immediately. '
                    f'(Configure SMTP to receive this by email instead.)',
                    'success'
                )
            return redirect(url_for('login'))
        conn.close()
        log_login_attempt(f'reset:{email}', success=False)
        # Same message whether or not email exists (avoid account enumeration)
        flash('If that email is registered, recovery instructions have been processed.', 'info')
        return redirect(url_for('login'))
...
def run_prediction():
    ...
    # Clean recs for json storage (keep full balanced mix up to ~230)
    clean_recs = []
    for r in recommendations[:240]:
...
# ---------------------------------------------------------------------------
# Admin Notifications, Calendar, Seat Matrix CRUD (Features 17, 18, 24)
# ---------------------------------------------------------------------------
...
def admin_notifications():
    ...
    if request.method == 'POST':
        ...
        alert_type = request.form.get('alert_type', 'normal')
        publish_date = request.form.get('publish_date') or datetime.now().strftime('%Y-%m-%d')
        if title and description:
            cursor.execute(
                "INSERT INTO notifications (title, description, alert_type, publish_date, is_active) VALUES (?, ?, ?, ?, 1)",
                (title, description, alert_type, publish_date)
            )
            conn.commit()
            flash('Notification published successfully.', 'success')
        else:
            flash('Title and description are required.', 'danger')
    cursor.execute("SELECT * FROM notifications ORDER BY publish_date DESC, id DESC")
    notes = cursor.fetchall()
    conn.close()
    return render_template('admin/notifications.html', notifications=notes)


@app.route('/admin/notifications/toggle/<int:note_id>', methods=['POST'])
@admin_required
def admin_toggle_notification(note_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE notifications SET is_active = CASE WHEN is_active = 1 THEN 0 ELSE 1 END WHERE id = ?", (note_id,))
    conn.commit()
    conn.close()
    flash('Notification status updated.', 'info')
...
def admin_calendar():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("SELECT COALESCE(MAX(stage_order), 0) + 1 FROM counselling_calendar")
        next_order = cursor.fetchone()[0]
        if stage_name:
            cursor.execute(
                "INSERT INTO counselling_calendar (stage_order, stage_name, start_date, end_date, status, description) VALUES (?, ?, ?, ?, ?, ?)",
                (next_order, stage_name, start_date, end_date, status, description)
            )
            conn.commit()
...
@app.route('/admin/seat-matrix/add', methods=['POST'])
@admin_required
def admin_add_seat_matrix():
    college_code = request.form.get('college_code', '').strip().upper()
    branch_code = request.form.get('branch_code', '').strip().upper()
    branch_name = request.form.get('branch_name', '').strip()
    intake = int(request.form.get('total_intake_seats') or 0)
    fees = int(request.form.get('fees') or 0)
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO college_branches (college_code, branch_code, branch_name, total_intake_seats, fees) VALUES (?, ?, ?, ?, ?)",
        (college_code, branch_code, branch_name, intake, fees)
    )
    conn.commit()
    conn.close()
    flash(f'Seat matrix entry added for {college_code}.', 'success')
    return redirect(url_for('admin_seat_matrix'))


@app.route('/admin/seat-matrix/delete/<int:entry_id>', methods=['POST'])
@admin_required
def admin_delete_seat_matrix(entry_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM college_branches WHERE id = ?", (entry_id,))
    conn.commit()
    conn.close()
    flash('Seat matrix entry removed.', 'info')
    return redirect(url_for('admin_seat_matrix'))


@app.route('/admin/colleges/edit/<college_code>', methods=['GET', 'POST'])
@admin_required
def admin_edit_college(college_code):
    conn = get_db_connection()
    cursor = conn.cursor()
    if request.method == 'POST':
        name = request.form.get('college_name', '').strip()
        district = request.form.get('district', '').strip().upper()
        region = request.form.get('region', 'AU').strip().upper()
        ctype = request.form.get('college_type', '').strip()
        website = request.form.get('website', '').strip()
        cursor.execute(
            "UPDATE colleges SET college_name=?, district=?, region=?, college_type=?, website=? WHERE college_code=?",
            (name, district, region, ctype, website, college_code)
        )
        conn.commit()
        conn.close()
        flash(f'College {college_code} updated.', 'success')
        return redirect(url_for('admin_colleges'))
    cursor.execute("SELECT * FROM colleges WHERE college_code = ?", (college_code,))
    college = cursor.fetchone()
    conn.close()
...
@app.route('/admin/branches/edit/<branch_code>', methods=['GET', 'POST'])
@admin_required
def admin_edit_branch(branch_code):
    conn = get_db_connection()
    cursor = conn.cursor()
    if request.method == 'POST':
        name = request.form.get('branch_name', '').strip()
        dept = request.form.get('department', 'Engineering').strip()
        cursor.execute("UPDATE branches SET branch_name=?, department=? WHERE branch_code=?", (name, dept, branch_code))
        conn.commit()
        conn.close()
        flash(f'Branch {branch_code} updated.', 'success')
        return redirect(url_for('admin_branches'))
    cursor.execute("SELECT * FROM branches WHERE branch_code = ?", (branch_code,))
...
@app.route('/admin/cutoffs/delete/<int:cutoff_id>', methods=['POST'])
@admin_required
def admin_delete_cutoff(cutoff_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM cutoffs WHERE id = ?", (cutoff_id,))
    conn.commit()
    conn.close()
    flash('Cutoff record deleted.', 'info')
    return redirect(url_for('admin_cutoffs'))


@app.route('/admin/cutoffs/edit/<int:cutoff_id>', methods=['GET', 'POST'])
@admin_required
def admin_edit_cutoff(cutoff_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    if request.method == 'POST':
        opening = int(request.form.get('opening_rank') or 0)
        closing = int(request.form.get('closing_rank') or 0)
        category = request.form.get('category', 'OC').strip()
        gender = request.form.get('gender', 'BOYS').strip()
        round_name = request.form.get('round', 'Phase 1').strip()
        cursor.execute(
            """UPDATE cutoffs SET opening_rank=?, closing_rank=?, category=?, gender=?, round=?
               WHERE id=?""",
            (opening, closing, category, gender, round_name, cutoff_id)
        )
        conn.commit()
        conn.close()
        flash('Cutoff record updated.', 'success')
        return redirect(url_for('admin_cutoffs'))
    cursor.execute("SELECT * FROM cutoffs WHERE id = ?", (cutoff_id,))
    row = cursor.fetchone()
    conn.close()
    if not row:
        flash('Cutoff not found.', 'danger')
        return redirect(url_for('admin_cutoffs'))
    return render_template('admin/edit_cutoff.html', cutoff=row)
...
# ---------------------------------------------------------------------------
# Change Password (Feature 1 completion)
# ---------------------------------------------------------------------------
...
def change_password():
    if request.method == 'POST':
        ...
        conn.commit()
        conn.close()
        flash('Password changed successfully. Please use the new password next time you sign in.', 'success')
        if session.get('role') == 'admin':
            return redirect(url_for('admin_dashboard'))
        return redirect(url_for('student_dashboard'))
...
if __name__ == '__main__':
    # Local development only. For production use:
