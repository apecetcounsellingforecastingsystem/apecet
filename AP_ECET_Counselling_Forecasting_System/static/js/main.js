...
// 4. OTP Verification via backend API (email & phone)
// Real delivery when SMTP/Twilio env vars are set; otherwise demo mode shows OTP on-screen.
async function sendOtp(type) {
  ...
  const otpRow = document.getElementById(`${type}OtpRow`);
  const otpInputEl = document.getElementById(`${type}OtpInput`);

  if (!val) {
    alert(`Please enter a valid ${type} first.`);
    return;
  }
  if (type === 'phone' && !/^\d{10}$/.test(val)) {
    alert('Enter a valid 10-digit mobile number.');
    return;
  }

  if (statusEl) statusEl.innerHTML = '<span style="color:var(--accent-cyan);">Sending OTP…</span>';
  ...
  try {
    const res = await fetch('/api/otp/send', {
      ...
      body: JSON.stringify({ channel: type, target: val, purpose: 'register' })
    });
    const data = await res.json();
    if (!data.success) {
      if (statusEl) statusEl.innerHTML = `<span style="color:#ef4444;">${data.message || 'Failed to send OTP'}</span>`;
    ...
    if (otpInputEl) {
      otpInputEl.value = data.demo_otp || '';
      otpInputEl.focus();
    }
    if (data.demo_otp) {
      if (statusEl) statusEl.innerHTML = `<span style="color:var(--accent-cyan);">Demo OTP: <b style="letter-spacing:2px;font-size:1.05em;">${data.demo_otp}</b> — enter & confirm</span>`;
    } else {
      if (statusEl) statusEl.innerHTML = `<span style="color:var(--accent-cyan);">OTP sent to your ${type}. Enter it below.</span>`;
    }
    if (sendBtn) {
    ...
    if (statusEl) statusEl.innerHTML = '<span style="color:#ef4444;">Network error. Try again.</span>';
    if (sendBtn) sendBtn.disabled = false;
  }
}
...
async function confirmOtp(type) {
  const inputId = type === 'email' ? 'regEmail' : 'regPhone';
  const val = document.getElementById(inputId)?.value.trim();
  const otpCode = document.getElementById(`${type}OtpInput`)?.value.trim();
  const statusEl = document.getElementById(`${type}OtpStatus`);
  const sendBtn = document.getElementById(`btnSend${type.charAt(0).toUpperCase() + type.slice(1)}Otp`);
  const otpRow = document.getElementById(`${type}OtpRow`);
  ...
  try {
    const vRes = await fetch('/api/otp/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ channel: type, target: val, otp: otpCode, purpose: 'register' })
    });
    ...
    if (vData.success) {
      ...
      if (statusEl) statusEl.innerHTML = `<span style="color: var(--color-safe); font-weight:600;">✓ ${type.toUpperCase()} Verified</span>`;
    } else {
      if (statusEl) statusEl.innerHTML = `<span style="color:#ef4444;">${vData.message || 'Incorrect OTP'}</span>`;
      alert(vData.message || 'Incorrect OTP');
    }
  } catch (err) {
    console.error('OTP verify error:', err);
    if (statusEl) statusEl.innerHTML = '<span style="color:#ef4444;">Network error.</span>';
  }
}
